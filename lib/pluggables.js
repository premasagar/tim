var Pluggables = (function(){
    ...
    /*
        Options:
            `exec`: The main function for the plugin.
            `context`: An optional `this` context for the `exec` function to be called with.
    ...
    */
    function Plugin(settings){
        ...
        if (typeof settings !== "object"){
            ...
            // `context` setting; this supplies a `this` context to each plugin `exec` call (see `Pluggables.morph` method)
    ...
    Pluggables.prototype = extend([], {
        ...
        create: function(settingsOrFunction){
            ...
            if (typeof settingsOrFunction === "function"){
                settingsOrFunction = {exec: settingsOrFunction};
            }
        ...
        indexOf: function(plugin){
            ...
            // if argument is a function, find the first plugin having the function as its `exec` method
            else if (typeof plugin === "function") {
                for (; i<len; i++){
                    if (plugin === this[i].exec) {
                        return i;
                    }
                }
            }
        ...
        trigger: function(/* ... arbitrary number of args ... */){
            ...
            for (; i<len; i++){
                plugin = this[i];
                plugin.exec.apply(plugin.context || plugin, arguments);
            }
        ...
        transform: function(stopOnFirstTransform /* , ... arbitrary number of args ... */){
            ...
            for (; i<len; i++){
                ...
                // Get result from plugin execution
                result = plugin.exec.apply(
        ...
        group: function(namespace, plugins /* , ...Any number of Plugin instances or settings objects */){
            ...
            if (!group){
                ...
                // TODO: need a way to pass in settings to new Pluggables - maybe only settings on construction? or if first obj is not a function and doesn't have `exec`
