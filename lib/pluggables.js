var Pluggables = (function(){
    ...
    */
    ...
    Pluggables.prototype = extend([], {
        ...
        // Find the first plugin to match the passed `subject`
        indexOfFirstMatch: function(subject, from, to){
            var i = from || 0;
            to || (to = this.length);
        ...
        transform: function(input /* , ... arbitrary number of args ... */){
            var args = toArray(arguments),
                output = input,
                parseRegex = this.parseRegex,
                pluginIndex = this.indexOfFirstMatch(input),
                plugin;
            ...
            while (pluginIndex > -1){
                ...
                output = plugin.fn.apply(
                    ...
                    // TODO: move to a `matches` property on a payload object
                ...
                if (typeof output !== "undefined" && input !== output){
                    input = args[0] = output;
                }
                
                pluginIndex = this.indexOfFirstMatch(input, pluginIndex + 1);
            }
        ...
        // Execute all plugins, in priority order, and allow plugins to transform the 1st argument
        //transform: function(/* ... arbitrary number of args ... */){
        /*    var i = 0,
                len = this.length,
            ...
            while (i < len){
                ...
                // Get result from plugin function
                output = plugin.fn.apply(
                    // Context, for `this` in callback
                    plugin.context || plugin,
                ...
                // If a result was given, then use it to replace the 1st argument and pass it on to the next plugin
                // If the plugin returns *anything* then the remaining plugins are then filtered on the results, until no more matches are made
                if (typeof output !== "undefined" && input !== output){
                    args[0] = output;
                    // Rinse and repeat
                    plugins = plugins.slice(i + 1).filter(output);
